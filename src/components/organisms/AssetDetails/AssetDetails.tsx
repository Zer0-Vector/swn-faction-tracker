...
const AssetDetailsComponent = ({ asset }: AssetDetailsProps) => {
  ...
  const dmgText = useCallback(
    (result: AssetAttackResult) =>
      result.type === "DAMAGE" ? result.damage : "Special",
    []
  );
  ...
  if (!isAsset(asset.name)) {
    ...
    return (
      <Typography color="error" fontWeight="bold" fontStyle="italic">
        Could not load asset info
      </Typography>
    );
  }
  ...
  return (
    <Card sx={cardSx} data-testid="asset-details">
      <Item sx={descriptionSx} data-testid="description">
        {description}
      </Item>
      <Item sx={attributeSx} data-testid="attribute">
        {attributeText}
      </Item>
      ...
      <LabeledItem label="Upkeep" xsLabel={6} data-testid="upkeep">
        {upkeepText}
      </LabeledItem>
      <LabeledItem label="Attack" xsLabel={4} data-testid="attack">
        {attackText}
      </LabeledItem>
      <LabeledItem label="Counter" xsLabel={6} data-testid="counter">
        {counterText}
      </LabeledItem>
    </Card>
  );
