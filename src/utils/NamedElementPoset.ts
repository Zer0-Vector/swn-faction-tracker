...
import {
  ISluggedOrderedSet,
  SluggedCopyOnWriteArrayPoset,
} from "./SluggedOrderedSet";
...
export type NamedEntityWithReadonlyProps<
  TProps,
  ReadonlyKeys extends keyof Named<TProps>,
...
> extends IObservable<NamedElementPosetAction<T>> {
  ...
  update<K extends keyof Omit<T, N | keyof SluggedEntity>>(
    id: string,
    key: K,
    value: T[K]
  ): NamedSluggedEntity;
...
type NamedElementFactoryFn<T extends NamedSluggedEntity, A> = (
  input: Prettify<NamedSluggedEntity & A>
) => T;
  ...
  implements INamedElementPoset<T, A, N>
{
  ...
  private readonly filterFunc?: FilterFuncGenerator<
    Omit<Named<Pick<A, M>>, "name">,
    T
  >;

  constructor(
    factory: NamedElementFactoryFn<T, A>,
    elements: T[] = [],
    filterFunc?: FilterFuncGenerator<Omit<Named<Pick<A, M>>, "name">, T>
  ) {
    super();
  ...
  update<K extends keyof Omit<T, N | keyof SluggedEntity>>(
    ...
    if (element === undefined) {
      throw new Error(
        `Cannot update ${String(key)}, unknown id=${id} keys: ${this.set.getAll().map((e) => e.slug)}`
      );
    }
    if (key === "name") {
