...
export type SelectionDialogOptions<T> = Omit<DialogOptions<T>, "children" | "fetchData">
  & {
    options: T[],
...
export function useSelectionDialog<T>() {
  ...
  const renderInput = useCallback((params: AutocompleteRenderInputParams) => <TextField {...params} />, []);

  return useCallback((options: SelectionDialogOptions<T>) => showDialog({
    title: options.title,
