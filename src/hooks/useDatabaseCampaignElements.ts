...
export type CampaignInfo = Prettify<
  Named<Identifiable> & {
    description?: string;
    sector?: SectorInfo;
  }
...
interface CampaignElementsWrapper {
...
export const useDatabaseCampaignElements = (
  campaignId: string = "(default)"
): CampaignElementsWrapper => {
  const db = useFirestore();
  const auth = useAuth();
  const [campaignData, setCampaignData] = useState<CampaignElementsWrapper>(
    {} as CampaignElementsWrapper
  );
  ...
  useEffect(() => {
    ...
    if (isMounted && uid) {
      const elements: Omit<CampaignElements, "campaign">
        & Partial<Pick<CampaignElements, "campaign">> = {
      ...
      getDoc(doc(db, "users", uid, "campaigns", campaignId))
        ...
        .then(() =>
          getDocs(
            collection(db, `/users/${uid}/campaigns/${campaignId}/elements`)
          )
        )
        .then((ss) => {
          ss.forEach((doc) => {
