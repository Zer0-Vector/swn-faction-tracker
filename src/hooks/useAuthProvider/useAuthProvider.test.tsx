...
function TestComp({ name }: { readonly name: string }) {
  ...
  return (
    <div>
      ...
      <button
        onClick={couldThrow(() => auth.login("email@test.com", "123456"))}
      >
        login
      </button>
      <button onClick={couldThrow(() => auth.logout())}>logout</button>
      <button
        onClick={couldThrow(() => auth.signup("email@signup.com", "789789"))}
      >
        signup
      </button>
      <button
      ...
      <button
        onClick={couldThrow(() => auth.sendPasswordResetEmail("pw@reset.com"))}
      >
        pw reset
      </button>
    </div>
...
describe("useAuthProvider", () => {
  ...
  it("calls setPersistence on mount", () => {
    ...
    expect(setPersistence).toBeCalledWith(
      expect.anything(),
      expect.objectContaining({ type: browserLocalPersistence.type })
    );
  });
  ...
  it("sends password reset email", async () => {
    ...
    expect(sendPasswordResetEmail).toBeCalledWith(
      expect.anything(),
      "pw@reset.com"
    );
  });
